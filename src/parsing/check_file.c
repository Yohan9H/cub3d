...
/*   Updated: 2024/10/30 13:32:41 by yohurteb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	svg_str_for_ray(t_data *data, char *line, char *id, int code)
{
	char	*str;
	int		i;
...
void	take_line(t_data *data, char *id, char *line)
{
	...
	if (ft_strncmp(id, "NO", 3) == 0)
	{
		svg_str_for_ray(data, line, id, data->pars->no);
		data->pars->no = 1;
	}
	else if (ft_strncmp(id, "SO", 3) == 0)
	{
		svg_str_for_ray(data, line, id, data->pars->so);
		data->pars->so = 1;
	}
	else if (ft_strncmp(id, "WE", 3) == 0)
	{
		svg_str_for_ray(data, line, id, data->pars->we);
		data->pars->we = 1;
	}
	else if (ft_strncmp(id, "EA", 3) == 0)
	{
		svg_str_for_ray(data, line, id, data->pars->ea);
		data->pars->ea = 1;
	}
	else
